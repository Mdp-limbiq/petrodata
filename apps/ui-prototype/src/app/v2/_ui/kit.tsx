...
/* ── Bloque de cifras ───────────────────────────────────────────────────
   ...
   Fui a buscar cifras de 15px para arriba en todo el sitio y hay exactamente
   dos —"−4.41%" y "+1.15%"—, las dos en este bloque:
   ...
   Dos cosas que corrige respecto de lo que teníamos:

   · La cifra iba a 21px, que es el tope del sistema. Ese tope se usa UNA sola
     vez en toda la referencia y es el título de la página; ninguna cifra llega
     ahí. Acá bajan a 17, que es donde la referencia sí las pone.

   · Eran tres cards, una por número. Tres planos separados para tres lecturas
     de la misma cosa. La referencia usa una card y adentro columnas flex-1,
     que es lo que dice que son: partes de un mismo resumen.

   Lo que NO se copia: el punto de color al lado del rótulo. Allá cada cifra
  ...
  return (
    <Card>
      ...
      <div className="flex items-start gap-4 p-3">
        {items.map((it) => (
          <div key={it.rotulo} className="min-w-0 flex-1">
            ...
            {/* El apoyo envuelve, no se recorta. A 375 la columna da 93px y
                ...
                esta cifra— se perdía en el ancho donde más importa. */}
            {it.apoyo && (
              <span className="s-mono block text-[11.5px]" style={{ color: 'var(--ink-3)' }}>
                {it.apoyo}
              </span>
            )}
          </div>
