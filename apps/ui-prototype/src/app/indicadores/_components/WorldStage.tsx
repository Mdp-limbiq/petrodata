...
import { formatFigure, formatDeltaPct, tierColor } from './format'
  ...
  return (
    <div className="border-t border-nd-border pt-8">
      ...
      {arg ? (
        <div className="mb-8 grid grid-cols-1 gap-px overflow-hidden border border-nd-border bg-nd-border sm:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="flex items-center justify-center bg-nd-surface px-6 py-4">
            <span className="text-2xl text-oil" aria-hidden>
              →
            </span>
            {jump != null && jump > 0 ? (
              <span className="ml-3 font-mono text-[11px] uppercase tracking-[0.06em] text-oil">
      ...
      <span className="mt-3 inline-block font-mono text-[10px] text-nd-text-disabled">
        {ranking.source.label} · Update {ranking.source.asOf}
      </span>
    </div>
  )
}
  ...
  return (
    <div className="bg-nd-surface px-6 py-5">
      ...
      <div className="mt-1 flex items-baseline gap-2">
        ...
        <span className="type-kpi !text-4xl md:!text-5xl" style={{ color }}>
          {rank}
        </span>
      </div>
      ...
      {rows.map((r, i) => {
        ...
        return (
          <div
            ...
            style={arg ? { background: 'color-mix(in srgb, var(--data-oil) 8%, transparent)' } : undefined}
          >
            <span
              className="font-mono text-[11px] tabular-nums"
              style={{ color: arg ? 'var(--data-oil)' : 'var(--nd-text-disabled)' }}
            >
            ...
            <div className="min-w-0">
              <div className="flex items-baseline justify-between gap-3">
                <span
                  ...
                  style={{ color: arg ? 'var(--data-oil)' : 'var(--nd-text-display)', fontWeight: arg ? 600 : 400 }}
                >
              ...
              <div className="mt-1.5 h-1.5 w-full overflow-hidden bg-nd-border">
                <div
                  ...
                  data-pct={pct}
                  className="h-full"
                  style={{ width: `${pct}%`, background: arg ? 'var(--data-oil)' : 'var(--nd-text-disabled)', opacity: arg ? 1 : 0.55 }}
                />
              </div>
            </div>
            <span
              className="font-mono text-[10px] uppercase tracking-[0.06em]"
              style={{ color: arg ? 'var(--data-oil)' : 'transparent' }}
            >
              {arg ? 'ARG' : '·'}
            </span>
          </div>
...
function GrowthBlock({ growth }: { growth: InvMundoGrowth }) {
  ...
  return (
    <div className="border-t border-nd-border pt-8">
          ...
          return (
            ...
            >
              ...
              <span
                ...
                style={{ color: arg ? 'var(--data-oil)' : 'var(--nd-text-secondary)' }}
              >
                {r.growthPct >= 0 ? '+' : ''}
      ...
      <span className="mt-3 inline-block font-mono text-[10px] text-nd-text-disabled">
        {growth.source.label} · Update {growth.source.asOf}
      </span>
    </div>
  )
}
...
function PolicyStrip({ politica }: { politica?: InvPolitica }) {
  ...
  return (
    <div className="border-t border-nd-border pt-8">
          ...
          {p.charts.map((c) => (
            <div key={c.id} className="bg-nd-surface p-5">
              <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
                <h4 className="text-sm font-semibold text-nd-text-display [font-family:var(--font-inter-tight)]">{c.title}</h4>
              ...
              <span className="mt-2 inline-block font-mono text-[10px] text-nd-text-disabled">
                {c.source.label} · Update {c.source.asOf}
              </span>
            </div>
          ))}
        </div>
      ...
      {/* The GDP payoff */}
      {p.impacto ? (
        <div className="mt-8 border border-oil/30 bg-nd-surface-raised/40 p-6 md:p-8">
          <span className="block font-mono text-[10px] uppercase tracking-[0.08em] text-oil">
            {t('world.impactKicker')}
          </span>
          <p className="type-display mt-3 max-w-2xl text-pretty !text-[1.3rem] !leading-[1.3] md:!text-[1.5rem]">
            {p.impacto.headline}
          </p>
...
function RigiBlock({ rigi }: { rigi: InvRigi }) {
  ...
  return (
    <div ref={ref} className="mb-10 border border-nd-border bg-nd-surface p-5 md:p-6">
      ...
      <span className="mt-3 inline-block font-mono text-[10px] text-nd-text-disabled">
        {rigi.source.label} · Update {rigi.source.asOf}
      </span>
    </div>
  )
}
  ...
  return (
    <div className="bg-nd-surface p-5">
      <div className="flex items-center justify-between gap-2">
        <span className="block font-mono text-[10px] uppercase tracking-[0.08em] text-oil">{lever.tag}</span>
        {hasChart ? (
